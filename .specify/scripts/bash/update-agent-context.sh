...
NEW_LANG=$(grep "^**Language/Version**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's#^**Language/Version**: ##g' | grep -v "NEEDS CLARIFICATION" || echo "")
NEW_FRAMEWORK=$(grep "^**Primary Dependencies**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's#^**Primary Dependencies**: ##g' | grep -v "NEEDS CLARIFICATION" || echo "")
...
fi; echo "✅ $agent_name context file updated successfully"; }
case "$AGENT_TYPE" in
